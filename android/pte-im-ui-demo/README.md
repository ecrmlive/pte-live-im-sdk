...
`PteIMUIDemoApplication` configures `apiDomain`、`imDomain`、`cosDomain` once at application startup. The login page only accepts the business backend's numeric SDK App ID, numeric user ID, and short-lived UserSig. It never persists UserSig or deployment secrets.
...
Debug builds automatically connect to the local `pte-live-im` Docker stack through `10.0.2.2`, request a fresh server-issued test UserSig, and retain only the selected test user ID, stable device ID, and explicit-logout flag. After a fresh Debug login it directly opens the `Work Team 工作群` visual-review conversation, so message types and interactions are immediately inspectable; press Back to enter the normal business shell. On first entry it seeds the real local IM API with `Pte Demo`、`Alice`、`Bob`、`Carol`、`Dave` mutual follows, four C2C conversations, and two groups. The generated UserSig remains memory-only; release builds retain HTTPS/WSS validation and use the business-login form.
