...
/**
 Demo-owned navigation for the login and registration flow. It owns language
 and theme selection for the sample application; reusable IM pages do not
 depend on it.
 */
@MainActor final class PteIMUIDemoAuthNavigationBar: UIView {
  ...
  private var languageMenuOverlay: PteIMUIDemoLanguageMenuOverlay?
  ...
  func apply(palette: PteIMUIThemePalette, themeMode: PteIMThemeMode, language: PteIMLanguage) {
    ...
    appearanceButton.setImage(UIImage(systemName: dark ? "moon.fill" : "sun.max"), for: .normal)
    appearanceButton.tintColor = palette.iconColor
  }
  ...
  static func applySystemBars(to controller: UIViewController, themeMode: PteIMThemeMode) {
  ...
  @objc private func showLanguageMenu() {
    ...
    let menu = PteIMUIDemoLanguageMenuOverlay(
      frame: window.bounds,
 ...
 */
@MainActor private final class PteIMUIDemoLanguageMenuOverlay: UIView {
