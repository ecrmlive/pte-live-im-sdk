...
public final class PteIMSDK: NSObject {
  ...
  public static func preview(baseConfig: PteIMBaseConfig, loginConfig: PteIMLoginConfig) throws -> PteIMSDK {
    try PteIMSDK(config: PteIMSessionConfig(base: baseConfig, login: loginConfig))
  }
  ...
  private func send(_ message: PteIMMessage) -> PteIMMessage {
    guard let conversationId = UInt64(message.conversationId), conversationId > 0 else {
      onError?(PteIMError.invalidConversationId)
      return message
    }
    var outgoing = message; outgoing.senderId = config.userId
    do { try store.enqueue(outgoing); onMessageStateChanged?(outgoing.clientMsgId, .pending); flushOutbox() }
    catch { onError?(error) }
    return outgoing
  }
  ...
  private func uploadAndSendMedia(conversationId: String, fileURL: URL, type: PteIMMessageType, voiceDurationMs: Int64? = nil, waveform: String? = nil, progress: @escaping (Int64, Int64) -> Void) -> PteIMMessage {
    ...
    var message = PteIMMessage(conversationId: conversationId, senderId: config.userId, type: type, state: .uploading)
