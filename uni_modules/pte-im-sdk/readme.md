...
当前 UTS Core 已公开 `recallMessage(serverMsgId, callback)` 与 `deleteMessage(serverMsgId, callback)`，并通过 `onMessageEvent` 透传撤回/删除事件。引用字段可从同步和接收消息读取，但尚未提供发送引用的公开 API；表情反应 REST 与实时状态尚未接入。请勿用本地缓存删除替代 `deleteMessage`，也不要让业务侧持久化表情反应作为 IM 真相源；完整差异见[消息生命周期](../../docs/message-lifecycle.md)。
